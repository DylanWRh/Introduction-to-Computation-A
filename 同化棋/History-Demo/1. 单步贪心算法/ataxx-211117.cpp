...
struct Case{int x1=0,y1=0,x2=0,y2=0,factors_mixed=-100;};//用于返回决策结果到ai_move()中
Case get_factors_mixed(int);//对当前棋局状况，判断多种因素的影响并计算权重，在ai_move()中，求factors_mixed时被调用
int can_absorb(int,int);//在ai_move()中被调用，判断走出一步之后能同化多少棋子
...
void ai_move(int color){
    ...
    **6. 离“核心”（同色棋子到该处距离之和最小）远的棋子尽可能扩张
    **7. 在棋子数已经较多时，5. 和6. 显得不那么必要
    */
        ...
        for(int p=-2;p<=2;p++){
            for(int q=-2;q<=2;q++){
                ...
                if(can_move_to(x2,y2)){
                    if(color!=cur_color)factors_mixed=can_absorb(x2,y2)+(2-move_judge(x1,y1,x2,y2));
                    if(color==cur_color){
                        factors_mixed=2*(can_absorb(x2,y2)+(2-move_judge(x1,y1,x2,y2)));
                        //留存副本，分析完成后恢复棋盘
                        ...
                        //走完之后，交换执手方，代入对方视角判断
                        Case competitor=get_factors_mixed(3-color);
                        factors_mixed=factors_mixed-competitor.factors_mixed;
                        for(int i=1;i<=7;i++){
